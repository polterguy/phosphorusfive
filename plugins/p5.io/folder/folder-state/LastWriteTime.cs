...
namespace p5.io.folder.folder_state
{
    ...
    public static class LastWriteTime
    {
        ...
        public static void p5_io_folder_get_last_write_time (ApplicationContext context, ActiveEventArgs e)
        {
            ObjectIterator.Iterate (context, e.Args, true, "read-file", delegate (string filename, string fullpath) {
                e.Args.Add (filename, new DirectoryInfo (fullpath).LastWriteTime);
            });
        }
    }
}
