...
namespace p5.mongodb
{
    ...
    public static class Update
    {
        ...
        [ActiveEvent (Name = "p5.mongo.update")]
        private static void p5_mongo_update (ApplicationContext context, ActiveEventArgs e)
        {
            var table = e.Args.Get<string> (context);
            if (string.IsNullOrEmpty (table)) // no table name given
                throw new ArgumentException ("[p5.mongo.update] needs the table name as the value of its node, either through an expression or a constant");
            ...
            if (XUtil.IsExpression (table)) {
                ...
                //    throw new ArgumentException ("if [p5.mongo.update] is given an expression, the expression needs to return only one value that can be converted into a string somehow");
            }
