    ...
    public static class Transform
    {
        ...
        private static void ApplyTransformations (ApplicationContext context, Node args, ref Bitmap bitmap)
        {
            ...
            foreach (var idxTransNode in args["transformations"].Children) {
                ...
                try {
                    context.Raise (activeEventName, idxTransNode);
        ...
        {
            var source = context.Raise (".p5.io.unroll-path", new Node ("", args.GetExValue<string> (context))).Get<string> (context);
            context.Raise (".p5.io.authorize.read-file", new Node ("", source).Add ("args", args));
        ...
        {
            var destination = context.Raise (".p5.io.unroll-path", new Node ("", args.GetExChildValue<string> ("destination", context))).Get<string> (context);
            context.Raise (".p5.io.authorize.modify-file", new Node ("", destination).Add ("args", args));
