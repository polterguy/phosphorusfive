        ...
        public static void p5_imaging_get_size (ApplicationContext context, ActiveEventArgs e)
        {
            ...
            using (new ArgsRemover (e.Args, true)) {
                ...
                foreach (var idxPath in XUtil.Iterate<string> (context, e.Args)) {
                    ...
                    var source = context.RaiseEvent (".p5.io.unroll-path", new Node ("", idxPath)).Get<string> (context);
                    context.RaiseEvent (".p5.io.authorize.read-file", new Node ("", source).Add ("args", e.Args));
