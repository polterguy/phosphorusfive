...
/// 
///     Contains all the Ajax functionality in Phosphorus, such as its Widgets, AjaxPage and so on. pf.ajax is a C# ASP.NET WebControls
///     library, that can be used without bringing in any other dependencies from Phosphorus.Five. In such a regard, pf.ajax is an alternative
///     to other Ajax Libraries, such as Anthem.NET and ASP.NET Ajax.
/// 
///     If all you wish to to, is to have a nice Ajax library for your C# or VB.NET ASP.NET projects, then pf.ajax is a decent alternative.
///     If this is what you want, then the only project dll you need to reference inside of your own projects is <em>"pf.ajax.dll"</em>.
/// 
///     pf.ajax is a managed Ajax Library, and all JavaScript for the library is roughly 5KB when built in Release Build. In addition, 
///     pf.ajax only sends the changes back from the server to your client, which means that the amount of bandwidth usage when using
///     pf.ajax is probably unmatched by other similar libraries.
/// 
///     With pf.ajax, you have complete control over what HTML is rendered from your server. There are no <em>"magic divs"</em>, or huge
///     JavaScript files being transfered. pf.ajax tries to fix what needs to be fixed, and leave the rest up to you. pf.ajax is
///     100% perfectly compatible with all other major Ajax Libraries out there, such as jQuery and Prototype.js.
/// 
///     With pf.ajax, you can create your WebControls, just like you would in a normal ASP.NET project, and let the Ajax parts, and updating
...
///     is rendered.
/// 
///     For examples of how to use pf.ajax, you can check out the example web project from the Phosphorus.Five download called 
///     <em>"pf.ajax.samples"</em>.
/// </summary>
namespace pf.ajax
{
    /// <summary>
    ///     Contains all core functionality in pf.ajax.
    /// 
    ///     Contains all the core helper and supporting features in pf.ajax, such as the Manager, AjaxPage and so on.
    ...
    namespace core
    {
        ...
        ///     you can implement the <see cref="pf.ajax.core.IAjaxPage">IAjaxPage</see> interface on your page instead,
        ///     and create an instance of the <see cref="pf.ajax.core.Manager" /> yourself, during the
