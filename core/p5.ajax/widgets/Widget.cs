...
namespace p5.ajax.widgets
{
    ...
    /// 
    ///     This is the abstract base class for all widgets in p5.ajax, and is what allows p5.ajax to update
    ...
    ///     that p5.ajax consumes very little bandwidth. This fact combined with the fact that p5.ajax only contains ~5KB of
    ...
    public abstract class Widget : Control, IAttributeAccessor
    {
        ...
        ///     <see cref="p5.ajax.widgets.Widget.RenderingType">RenderingType</see>, to understand how this affects your rendered HTML.
        /// </summary>
        ...
        protected override void OnLoad (EventArgs e)
        {
            ...
            if (IsPhosphorusRequest) {
                if (Page.Request.Params ["__p5_widget"] == ClientID) {
                    Page.LoadComplete += delegate {
                        // event was raised for this widget
                        InvokeEventHandler (Page.Request.Params ["__p5_event"]);
                    };
                }
            }
            base.OnLoad (e);
